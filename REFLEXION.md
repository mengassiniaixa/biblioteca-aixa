...
- ~~**Sin healthcheck HTTP en el backend.** Compose usa `depends_on: db healthy` para el orden de arranque, pero no chequea el `/health` del backend. Si el backend arranca en un estado degradado, compose no lo detecta.~~ *Cerrado 2026-09-25: el service `backend` tiene healthcheck que pega a `/health` con `wget --spider` (BusyBox en `node:20-alpine`, sin dependencias extra), y `frontend` depende del backend `service_healthy`, no solo del proceso up. `start_period: 15s` cubre migraciones + seed inicial del modo `pg`.*
...
- ~~**Sin CI/CD.** No hay pipeline que corra `docker build` en push a `main`. Todo local.~~ *Cerrado 2026-09-25: workflow `.github/workflows/ci.yml` con typecheck + tests (dominio + backend + frontend) y build de las dos imágenes docker en push a `main` y PRs. Sigue sin haber CD (deploy automático) porque no hay ambiente de destino.*
...
4. ~~**Tests de integración del backend HTTP** — hoy solo hay tests del dominio. Un `supertest` sobre `buildApp()` con el container real (in-memory alcanza) cubriría el mapeo HTTP → use-case → error.~~ *Cerrado 2026-09-25: 44 tests con Jest + supertest en `apps/backend/src/__tests__/`. Helper `buildTestApp()` monta el container en modo `memory` y expone factories `seedLibrarian`/`seedMember`/`createBook`. Cubre auth (6), books (9), loans (10), reservations (7), stats (3) y errorHandler + CORS + /health (9). Corre con `yarn backend:test`.*
...
13. ~~**Healthcheck HTTP del backend en compose** para que `frontend` no arranque hasta que el backend responda `/health`, no solo hasta que el proceso levantó.~~ *Cerrado 2026-09-25 (ver sección "Scope consciente fuera de alcance" de Etapa 3).*
14. ~~**CI mínimo en GitHub Actions** — al menos `yarn test` de dominio + `yarn frontend:test` + `docker build` de ambas imágenes en push a `main`.~~ *Cerrado 2026-09-25: workflow `.github/workflows/ci.yml` con dos jobs paralelos en push a `main` y en PRs. Job `test` corre typecheck + los 3 test suites (dominio, backend, frontend); job `docker` buildea las dos imágenes.*
